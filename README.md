...
当前测试覆盖：规则引擎（`test_rule_engine.py`）+ 图编排（`test_graph.py`），共 42 个用例。

> ⚠️ `test_yi_completes_with_80` / `test_bing_interrupts_then_resumes_to_80` 有偶发失败，原因是 LLM 抽取非确定性（`degree_level` 会抽空），与代码无关，重跑即过。
...
1. 名字精确匹配名单或别名（147 所 + 109 别名）
2. 含 location suffix 的（如「中国石油大学（华东）」）同样匹配
3. 名单命中 + 「校区 / 研究生院 / 研究院 / 分校 / 学部」后缀 → 双一流
4. 名单命中 + 「学院」后缀 → 独立学院（非双一流）
5. LLM 结论与名单冲突，或两边都无结论 → 转人工复核
...
| 校内学院假阴性 | 「北京大学光华管理学院」等后缀含「学院」会被判 8 分，靠 LLM 冲突校验兜底转人工复核 |
| 项目非 git 仓库 | 目录下无 `.git`，变更记忆改为手动记录在 `docs/ai/changes/` |
